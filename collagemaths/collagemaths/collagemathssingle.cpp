...
string** best_patch_finder(int start, int end);
...
int main()
{
	...
	string** data = best_patch_finder(0, target_patch_matrix.size());
	int z;
	z = 0;
	...
	for (int i = 0; i < target_patch_matrix.size(); i++){
		...
		string x_coords   = data[i][0];
		string y_coords   = data[i][1];
		string best_patch = data[i][2];
...
string** best_patch_finder(int start, int end){
	...
	string** answer_data;
    answer_data = new string*[end-start];
