...
#REMOVED TIME SAVER WITH OR FALSE
    ...
    for a in range(0,number_of_patches_width):
        for b in range(0, number_of_patches_height):
            if str(a)+"-"+str(b) in answer_as_list and (a,b) not in done:
                ...
                if best_photo_name[0] != "c" and best_photo_name[0] != "t":
                    print(best_photo_name, "is used")
...
def collegify(file_name):
    answer = collagify(file_name,number_of_patches_width,number_of_patches_height)
    paste(answer,"com" + file_name.split("\\")[-1])
...
files = glob.glob(str(pathlib.Path().resolve()) + "\\computation\\patch_sized_photos\\*.*")
for f in files:
    os.remove(f)
